...
async def test_70050(browser_args):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # 测试步骤1: 登录 (操作次数: 1)
            with allure.step("测试步骤1: 登录"):
                log_info(f"开始测试步骤1 登录 的操作==============")
                
                # 公共断言方法，断言元素是否存在
                with allure.step("测试步骤1: 公共断言元素是否存在"):
                    await ui_operations.elem_assert_exists("//*[@id='username']")

                with allure.step("测试步骤1: 登录 - login 操作 (//*[@id='username'])"):
                    time.sleep(1)
                ...
                for attempt in range(1):
                        ...
                        try:
                            log_info(f"执行第{attempt + 1}次操作: login on //*[@id='username']")
                            # 使用安全操作机制，带重试
                            await ui_operations.elem_login("//*[@id='username']","//*[@id='password']","//*[@id='submit-button']","yanglingzhen@qq.com","azhen")
            ...
            # 最终检查浏览器状态
            if await ui_operations.is_browser_closed():
                log_info(f"检测到浏览器已关闭，test_70050 测试被用户中断")
                raise Exception("BROWSER_CLOSED_BY_USER")
